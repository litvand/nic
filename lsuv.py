# https://github.com/glassroom/torch_lsuv_init/blob/master/torch_lsuv_init.py
...
def LSUV_(
    ...
    if do_ortho_init:
        logging_FN(
            "Applying orthogonal init (zero init if dim < 2) to params in"
            f" {len(matched_modules)} module(s)."
        )
    ...
    with torch.no_grad():
        for i, m in enumerate(matched_modules):
            ...
            logging_FN(
                f"Module {i:2} after {(t+1):2} itr(s) | Mean:{mean:7.3f} |"
                f" Std:{std:6.3f} | {type(m)}"
            )
